...
- Extract: **company name**, **role title**, **department** (if mentioned), **location**, and **language** of the posting (e.g. Spanish or English).
...
- **Match the language of the job posting** (Spanish posting -> Spanish CV, English posting -> English CV). **Default to Spanish** if the posting language is ambiguous or mixed.
...
**3. Keyword coverage.** Reuse the required/preferred keyword list you extracted in Step 1 — do not re-derive it. Match each keyword against the extracted text, **in the posting's language**. When the CV is written in the same language as the posting (the default), matches are usually direct. If the CV language differs from the posting (e.g. an English CV for a Spanish posting), count concepts covered in the other language as synonym-only and note the language difference. Report a table:
